...
function hashPassword(plain) {
  ...
  const hash = import_crypto.default.scryptSync(String(plain), salt, KEYLEN, { N: SCRYPT_N, r: SCRYPT_R, p: SCRYPT_P });
  return `scrypt$${SCRYPT_N}$${SCRYPT_R}$${SCRYPT_P}$${salt.toString("base64")}$${hash.toString("base64")}`;
}
function verifyPassword(entered, stored) {
  if (!stored) return false;
  if (!isHashed(stored)) return entered === stored;
  try {
    const [tag, n, r, p, saltB64, hashB64] = stored.split("$");
    if (tag !== "scrypt") return false;
    const salt = Buffer.from(saltB64, "base64");
    const expected = Buffer.from(hashB64, "base64");
    const actual = import_crypto.default.scryptSync(String(entered), salt, expected.length, { N: Number(n), r: Number(r), p: Number(p) });
    return import_crypto.default.timingSafeEqual(actual, expected);
  } catch {
    return false;
  }
}
...
function verifySessionToken(token) {
  ...
  const expected = import_crypto.default.createHmac("sha256", secret()).update(body).digest("base64url");
...
function getEpoch(db, kind, sub) {
  try {
    if (kind === "ESS") {
      const emp = (db.data?.employees || []).find((e) => e.id === sub);
      return Number(emp?.session_epoch || 0);
    }
    const user = (db.data?.users || []).find((u) => u.username === sub || u.id === sub);
...
var PayrollDatabase = class _PayrollDatabase {
  ...
  createTables() {
    ...
    this.dbSqlite.run(`CREATE TABLE IF NOT EXISTS system_settings (
      ...
      const hashedDefault = import_crypto2.default.createHash("sha256").update("1234").digest("hex");
  ...
  resolveHodIdToEmployeeId(hodValue) {
    ...
    if (emp.reporting_hod) {
      ...
      if (resolved && resolved.id !== emp.reporting_hod) {
        ...
        try {
          ...
          console.warn("[HOD-AUTO-NORMALIZE] Failed to persist:", e);
        }
  ...
  normalizeAllReportingHods() {
    ...
    for (const emp of allEmps) {
      ...
      const resolved = this.resolveHodIdToEmployeeId(emp.reporting_hod);
      if (!resolved) {
        failed.push(`${emp.id}: could not resolve '${emp.reporting_hod}'`);
        continue;
      }
      if (resolved.id === emp.reporting_hod.replace(/\s+$/, "")) {
        alreadyCorrect++;
        continue;
      }
      emp.reporting_hod = resolved.id;
      ...
      try {
        ...
        failed.push(`${emp.id}: sqlite update failed`);
      }
      normalized++;
    }
    this.persistData();
  ...
  addLeaveApplication(app) {
    ...
    if (_s && _e) {
      const overlap = (this.data.leave_applications || []).find((o) => o.employee_id === app.employee_id && ["PENDING_HOD", "PENDING_HR", "APPROVED", "HOD_APPROVED"].includes(o.status) && String(o.start_date || "").slice(0, 10) <= _e && String(o.end_date || "").slice(0, 10) >= _s);
      if (overlap) {
    ...
    if (emp) {
      const balanceKey = app.leave_type === "CO" ? "leave_balance_compoff" : `leave_balance_${String(app.leave_type).toLowerCase()}`;
  ...
  updateLeaveStatus(id, status) {
    ...
    if (!app) return false;
    if (["APPROVED", "REJECTED", "REJECTED_HOD", "REJECTED_HR", "CANCELLED"].includes(app.status) || app.balance_deducted) {
      return false;
    }
    app.status = status;
    if (status === "APPROVED") {
  ...
  calculateSingleSlip(emp, att, month, forceFresh = false) {
...
Sakar & SVN Group`;
  ...
  verifyEmployeePassword(employeeId, entered) {
    const emp = this.getEmployeeById(employeeId);
    if (!emp) return { ok: false, isHashed: false };
    const stored = emp.password || emp.id;
  ...
  migrateEmployeePasswordToHash(employeeId, plainUsed) {
    const emp = this.getEmployeeById(employeeId);
   ...
   */
  cancelLeaveRequest(id, actor, reason) {
    const app = (this.data.leave_applications || []).find((a) => a.id === id);
    ...
    if (actor.role === "ESS" || !actor.isHR && isOwner) {
      ...
      if (app.status === "APPROVED") {
        ...
        app.cancellation_reason = String(reason);
        app.cancellation_requested_by = actor.sub;
        app.cancellation_requested_date = (/* @__PURE__ */ new Date()).toISOString();
        this.persistData();
    ...
    const posted = !!app.balance_deducted;
    const months = [];
    if (app.start_date) {
      const s = new Date(app.start_date);
      const e = new Date(app.end_date || app.start_date);
    ...
    if (posted) {
      ...
      if (emp) {
        if (app.leave_type === "CO") {
          ...
          const leaveKey = `leave_balance_${String(app.leave_type).toLowerCase()}`;
    ...
    app.cancellation_history = app.cancellation_history || [];
    app.cancellation_history.push({ by: actor.sub, role: actor.role, date: (/* @__PURE__ */ new Date()).toISOString(), reason: String(reason), postedReversed: posted });
    app.status = "CANCELLED";
    app.cancelled_by = actor.sub;
    app.cancelled_date = (/* @__PURE__ */ new Date()).toISOString();
    app.cancellation_reason = app.cancellation_reason || String(reason);
    this.persistData();
    return { ok: true, code: "CANCELLED" };
  }
  /** Mirror of autoUpdateAttendanceForLeave: remove posted leave days from the start-month record. */
  reverseAttendanceForLeave(employeeId, leaveDays, startDate, leaveType) {
    if (!employeeId || !startDate) return;
    const d = new Date(startDate);
    const month = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;
    const att = this.data.attendance.find((a) => a.employee_id === employeeId && a.month === month);
    if (!att) return;
    ...
    if (leaveType) {
      ...
      if (bucket) att[bucket] = Math.max(0, (att[bucket] || 0) - leaveDays);
    }
    att.working_days = (att.present || 0) + (att.weekly_off || 0) + (att.paid_holiday || 0) + (att.leave || 0);
    att.lop_days = (att.absent || 0) + (att.lwp || 0);
    att.total_days = (att.present || 0) + (att.absent || 0) + (att.weekly_off || 0) + (att.paid_holiday || 0) + (att.leave || 0) + (att.lwp || 0);
  ...
  applyOpeningBalance(rows, meta) {
    ...
    for (const r of rows) {
      ...
      if (!emp) {
        skipped.push({ id: r.id, why: "EMPLOYEE_NOT_FOUND" });
        continue;
      }
      if (emp.status && emp.status !== "ACTIVE") {
        skipped.push({ id: r.id, why: "NOT_ACTIVE" });
        continue;
      }
      const prev = { pl: emp.leave_balance_pl || 0, cl: emp.leave_balance_cl || 0, sl: emp.leave_balance_sl || 0, co: emp.leave_balance_compoff || 0 };
      if (r.pl !== void 0) emp.leave_balance_pl = Number(r.pl);
      if (r.cl !== void 0) emp.leave_balance_cl = Number(r.cl);
      if (r.sl !== void 0) emp.leave_balance_sl = Number(r.sl);
      if (r.co !== void 0) emp.leave_balance_compoff = Number(r.co);
      emp.leave_opening = { pl: r.pl ?? prev.pl, cl: r.cl ?? prev.cl, sl: r.sl ?? prev.sl, co: r.co ?? prev.co, as_on: meta.asOn, source: "OPENING_BALANCE", batch_id: meta.batchId };
      emp.leave_opening_entered_by = meta.by;
      emp.leave_opening_entered_at = (/* @__PURE__ */ new Date()).toISOString();
    ...
    this.data.leave_import_batches.push({ id: meta.batchId, kind: "OPENING_BALANCE", filename: meta.filename, file_hash: meta.fileHash, as_on: meta.asOn, rows: rows.length, updated, skipped: skipped.length, by: meta.by, at: (/* @__PURE__ */ new Date()).toISOString() });
    this.persistData();
    return { updated, skipped };
  }
  /** Idempotency (OB-7): same normalized file content must never double-apply. */
  findImportBatchByHash(fileHash) {
    return (this.data.leave_import_batches || []).find((b) => b.file_hash === fileHash);
  }
  /** Historical leave import (T2): records only — balances come from recompute, NO attendance posting, NO payroll touch. */
  importHistoricalLeaves(rows, meta) {
    const skipped = [];
    let imported = 0;
    if (!this.data.leave_import_batches) this.data.leave_import_batches = [];
    let nextNum = Math.max(...(this.data.leave_applications || []).map((a) => parseInt(String(a.id).replace("LV", "")) || 0), 0);
    for (const r of rows) {
      const emp = this.getEmployeeById(r.employee_id);
      if (!emp) {
        skipped.push({ employee_id: r.employee_id, why: "EMPLOYEE_NOT_FOUND" });
        continue;
      }
      const type = String(r.leave_type || "").toUpperCase();
      if (!["PL", "CL", "SL", "CO"].includes(type)) {
        skipped.push({ employee_id: r.employee_id, why: `BAD_TYPE:${type}` });
        continue;
      }
      nextNum++;
      const rec = {
        ...
        status: "APPROVED",
        applied_date: (/* @__PURE__ */ new Date()).toISOString(),
        historical: true,
        source: "HISTORICAL_LEAVE",
        balance_deducted: false,
        // informational record — recompute handles balances
        hr_id: meta.by,
        hr_approved_date: (/* @__PURE__ */ new Date()).toISOString(),
    ...
    this.data.leave_import_batches.push({ id: meta.batchId, kind: "HISTORICAL_LEAVE", filename: meta.filename, file_hash: meta.fileHash, rows: rows.length, imported, skipped: skipped.length, by: meta.by, at: (/* @__PURE__ */ new Date()).toISOString() });
    this.persistData();
   ...
   */
  getLeaveRegisterSummary(company) {
    ...
    for (const emp of this.data.employees || []) {
      ...
      const ob = emp.leave_opening || { pl: emp.leave_balance_pl || 0, cl: emp.leave_balance_cl || 0, sl: emp.leave_balance_sl || 0, co: emp.leave_balance_compoff || 0, as_on: "2026-04-01", source: "OPENING_BALANCE" };
      const taken = { PL: 0, CL: 0, SL: 0, CO: 0 };
      for (const a of this.data.leave_applications || []) {
        if (a.employee_id !== emp.id || a.status !== "APPROVED" || a.cancellation_status === "REQUESTED") continue;
        ...
        if (taken[t] !== void 0) taken[t] += Number(a.days) || 0;
      }
      const row = { employee_id: emp.id, name: emp.name, company: emp.company, as_on: ob.as_on, source: ob.source };
      for (const t of ["PL", "CL", "SL", "CO"]) {
        const open = Number(ob[t === "CO" ? "co" : t.toLowerCase()] || 0);
        const balKey = t === "CO" ? "leave_balance_compoff" : `leave_balance_${t.toLowerCase()}`;
        row[t] = { opening: open, credit: 0, taken: taken[t], adjustment: 0, closing: Math.max(0, open - taken[t]), live_balance: Number(emp[balKey] || 0) };
      }
      out.push(row);
    }
    return out;
  ...
  addSalaryRevision(rev) {
    ...
    if (emp) {
      ...
      if (effectiveDate <= today) {
        ...
        if (rev.hra !== void 0 && rev.hra !== null) {
        ...
        if (rev.special_allowance !== void 0 && rev.special_allowance !== null) {
        ...
        emp.da = 0;
        if (rev.conveyance_allowance !== void 0 && rev.conveyance_allowance !== null) {
        ...
        if (rev.edu_allowance !== void 0 && rev.edu_allowance !== null) {
          emp.edu_allowance = Number(rev.edu_allowance);
        } else {
          const ratio = Number(rev.old_salary) > 0 ? Number(rev.new_salary) / Number(rev.old_salary) : 1;
          emp.edu_allowance = Math.round((emp.edu_allowance || 0) * ratio);
        }
        if (rev.medical_allowance !== void 0 && rev.medical_allowance !== null) {
...
async function createApp(supabaseAdmin) {
  ...
  app.use((req, res, next) => {
    ...
    if (payload.kind === "ESS") {
      const p = req.path;
      const adminSurface = p.startsWith("/api/admin/") || p.startsWith("/api/restore") || p.startsWith("/api/backup") || /^(\/api\/(hr\/users|hods|settings\/|revisions|payroll|leave-opening|historical-leaves|loans|employees|attendance))/.test(p) && req.method !== "GET" || p === "/api/employees" || p === "/api/backup-json" || /^\/api\/attendance-corrections/.test(p) && req.method !== "GET" && false || /^\/api\/leaves\/(workflow|status)/.test(p) && req.method !== "GET";
  ...
  app.get("/api/employees", (req, res) => {
    const { company } = req.query;
    const allowed = getAllowedCompanies(req);
    const stripCreds = (list) => (Array.isArray(list) ? list : []).map((e) => {
  ...
  app.post("/api/employee/change-password", async (req, res) => {
    try {
      ...
      const updated = db.updateEmployee(employee.id, {
        ...
        session_epoch: Number(employee.session_epoch || 0) + 1
      });
  ...
  app.post("/api/leaves/workflow", async (req, res) => {
    try {
      ...
      if (override && action === "APPROVE" && String(overrideReason || "").trim()) {
        try {
          const app0 = db.data?.leave_applications?.find((a) => a.id === id);
          if (app0) {
            app0.hr_override_reason = String(overrideReason).trim();
            app0.hr_override_by = String(actorId || req.headers["x-operator-name"] || "HR");
          }
        } catch {
        }
      }
  ...
  app.put("/api/employees/:id/leave-opening", async (req, res) => {
    try {
      ...
      emp.leave_opening = { pl: emp.leave_balance_pl, cl: emp.leave_balance_cl, sl: emp.leave_balance_sl, co: emp.leave_balance_compoff || 0, as_on: "2026-04-01", source: "OPENING_BALANCE", batch_id: "MANUAL" };
      emp.leave_opening_entered_by = String(getOperator(req));
      emp.leave_opening_entered_at = (/* @__PURE__ */ new Date()).toISOString();
  ...
  app.post("/api/leave-opening/import", async (req, res) => {
    try {
      ...
      const fileHash = import_crypto3.default.createHash("sha256").update(JSON.stringify(normalized)).digest("hex");
      ...
      const result = db.applyOpeningBalance(normalized, {
        batchId,
        fileHash,
        filename: String(filename || "opening-balance.csv"),
        by: String(getOperator(req)),
        asOn,
  ...
  app.post("/api/historical-leaves/import", async (req, res) => {
    try {
      ...
      const fileHash = import_crypto3.default.createHash("sha256").update(JSON.stringify(normalized.sort((a, b) => a.employee_id.localeCompare(b.employee_id)))).digest("hex");
      const existing = db.findImportBatchByHash(fileHash);
  ...
  app.post("/api/leaves/cancel", async (req, res) => {
    try {
      ...
      const actor = isESSSession ? { role: "ESS", sub: req.ess.sub, isHR: false } : { role: operatorRole, sub: req.ess?.sub || String(getOperator(req)), isHR: isHRLevel };
      const result = db.cancelLeaveRequest(String(leave_id), actor, String(reason));
      if (!result.ok) {
        const statusMap = { NOT_FOUND: 404, ALREADY_TERMINAL: 409, REASON_REQUIRED: 400, FORBIDDEN: 403, PAYROLL_PERIOD_CLOSED: 409 };
...
HR Department`;
  ...
  app.get("/api/backup-json", (req, res) => {
    try {
      ...
      if (Array.isArray(scrubbed.users)) scrubbed.users = scrubbed.users.map(({ password: _p, ...u }) => u);
      if (Array.isArray(scrubbed.employees)) scrubbed.employees = scrubbed.employees.map(({ password: _p, ...e }) => e);
      res.json(scrubbed);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
