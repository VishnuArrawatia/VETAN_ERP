...
- [x] **Profile-revert ROOT-CAUSE FIX (08-Sep, `storeMerge.ts` + `db.ts`):** (1) employee records ab `updated_at` merge-stamp lete hain (insertEmployee/updateEmployee) — merge har conflict me genuinely-newer record rakhta hai (one-sided-timestamp rule); (2) OCC conflict-merge me `_dirtyEmployeeIds` snapshot/re-apply — in-flight human edits stale-instance merge se bachte hain; (3) `persistData()` single-flight (duplicate concurrent cloud-write nahi); (4) reload timestamp-guards pehle se theek. **Photo:** base64 employee-record ke andar hi travel karta hai (Supabase blob me persist) — koi alag file-path nahi, koi Vercel-fs nahi. 42/42 tests pass (`scripts/vetan-task-tests.ts` — OCC conflict simulation included). **Deploy pending: `git push` (production deploy)**
- [x] **Form 16 NEW-REGIME (08-Sep, `server/form16-engine.ts` + `db.ts` + `Form16Portal.tsx`):** income components — Salary / Bonus / Arrear / Leave Encashment (10(10AA) exemption: least of actual, 10× avg monthly salary, ₹25L cap) / Other — sab ACTUAL payslip+FF data se, per-month, no double-count; tax working config-driven (`REGIME_CONFIGS` — FY-2026-27 slabs 0-4L nil→30%, std-ded 75k, 87A rebate 12L/60k, surcharge+cess, marginal relief); UI me FY-selector + slab-table + month-wise breakdown + TDS/balance. `?fy=` param API me. Purane flat-slab engine ka koi historical data modify nahi hua (calculation on-the-fly hai)
- [ ] `employees.json` vs live store drift (10 only-in-dump, 5 only-in-store)
- [x] VETAN changes **commit ho gaye** (2026-09-07, `28f5db3`): persistence-hardening — atomic JSON writes, daily cloud snapshots, safer auto-restore + shipped-snapshot sync. Workforce (`4d1240c`), MIS (`3b5f92d`) aur docs/scripts (`025eea5`) alag commits me. **⚠️ Push NAHI kiya — push = Vercel production deploy, sirf ready hone par**
