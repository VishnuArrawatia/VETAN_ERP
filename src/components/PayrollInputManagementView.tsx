...
export default function PayrollInputManagementView({
  ...
  useEffect(() => {
    ...
    monthSlips.forEach(s => {
      initialGrid[s.id] = {
        pay_days: s.pay_days ?? (() => { const d = new Date(parseInt(activeMonth.split('-')[0]), parseInt(activeMonth.split('-')[1]), 0).getDate(); return d - (s.lop_days || 0); })(),
      ...
      try {
        ...
        const parsedRecords = rows.map((r, idx) => {
          ...
          return {
            ...
            salary_advance: Number(r['Salary Advance (₹)'] || r['Salary Advance'] || r['Advance'] || 0),
            arrear_payment: Number(r['Arrear Payment (₹)'] || r['Arrear'] || 0),
    ...
    <div className="space-y-6">
      ...
      {subTab === 'INPUTS' && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-auto max-h-[70vh]">
                ...
                {filteredSlips.map((slip) => {
                  ...
                  return (
                    <tr key={slip.id} className="hover:bg-slate-50 transition font-mono">
                      ...
                      {/* Present / LOP — Editable */}
                      <td className="p-1 text-center font-sans bg-green-50/30">
                        <div className="flex items-center justify-center gap-1">
                          <input type="number" min={0} max={31}
                            value={rowInputs.pay_days ?? slip.pay_days ?? (() => { const d = new Date(parseInt(activeMonth.split('-')[0]), parseInt(activeMonth.split('-')[1]), 0).getDate(); return d - (slip.lop_days || 0); })()}
                          ...
                          <input type="number" min={0} max={31}
                            ...
                            title="LOP Days"
                          />
                          <span className="text-[8px] text-gray-400 block">d</span>
                        </div>
                      </td>
