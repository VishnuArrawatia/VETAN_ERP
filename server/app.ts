...
export async function createApp(supabaseAdmin?: any) {
  ...
  app.post('/api/attendance/bulk', (req, res) => {
    try {
      ...
      // Attendance can ALWAYS be saved — even when payroll is locked.
      // Payroll lock only prevents salary re-calculation, not attendance data entry.
  ...
  // ═══════════════════════════════════════════════════
  // BULK LEAVE UTILIZATION — Month-wise entry for HR
  // ═══════════════════════════════════════════════════

  // POST /api/leave-utilization-bulk
  // Body: { month: '2026-04', company: 'SVN-1', entries: [
  //   { employee_id, pl_days, cl_days, sl_days, compoff_days }
  // ]}
  app.post('/api/leave-utilization-bulk', async (req, res) => {
    try {
      ...
      for (const entry of entries) {
        const emp = allEmps.find((e: any) => e.id === entry.employee_id);
        if (!emp) continue;
        ...
        // Recalculate working_days and lop_days
  ...
  // GET /api/leave-utilization?month=2026-04&company=SVN-1
  // Returns current month's leave utilization for all employees
  app.get('/api/leave-utilization', (req, res) => {
    try {
      ...
      const utilization = filteredEmps.map((emp: any) => {
        const att = db.getAttendance().find((a: any) => a.employee_id === emp.id && a.month === month);
        return {
          employee_id: emp.id,
  ...
  app.get('/api/leave-register', (req, res) => {
    try {
      ...
      // Build leave summary from ALL active employees (not just attendance records)
      const allEmps = db.getEmployees();
      const filteredEmps = company && company !== 'ALL'
        ? allEmps.filter((e: any) => e.company === company)
        : allEmps;

      const leaveSummary = filteredEmps
...
HR Department`;
  ...
  app.put('/api/payslips/:id', async (req, res) => {
    try {
      ...
      // CRITICAL: Await Supabase persist before responding
      await db.persistDataSync();
  ...
  app.post('/api/daily-attendance/mark-all-present', (req, res) => {
    try {
      ...
      const employees = db.getEmployees().filter(e => 
        e.status === 'ACTIVE' && 
        (company === 'ALL' || !company || e.company === company)
      );
      ...
      for (const emp of employees) {
        ...
        // Mark present for this date (increment by 1)
  ...
  app.post('/api/daily-attendance/mark-employee', (req, res) => {
    try {
      ...
      if (status === 'PRESENT') {
        att.present = (att.present || 0) + 1;
        if (halfDay) att.lop_days = (att.lop_days || 0) + 0.5; // Half day LOP
  ...
  app.get('/api/daily-attendance/summary', (req, res) => {
    try {
      const { date, company } = req.query as { date?: string; company?: string };
      if (!date) return res.status(400).json({ error: 'Date required' });
  ...
  app.get('/api/excel/export/payroll/:month', (req, res) => {
    ...
    for (const s of slips) {
      const emp = db.getEmployeeById(s.employee_id);
      // Show FULL MONTH rate heads (not prorated earned) to match HR Excel format
  ...
  app.get('/api/reconciliation/:month', (req, res) => {
    try {
      ...
      let employees = db.getEmployees(company && company !== 'ALL' ? company : undefined);
      ...
      const payslips = db.getPayslipsByMonth(month, company && company !== 'ALL' ? company : undefined);
